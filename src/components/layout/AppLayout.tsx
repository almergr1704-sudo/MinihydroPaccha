...
export function AppLayout() {
  ...
  const baseNavFiltered = userRole === 'OPERATOR' 
    ...
    : userRole === 'VOCAL'
    ? baseNavigation.filter(nav => ['Reportes', 'Configuración'].includes(nav.name))
    : userRole === 'TESORERO'
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-3 truncate max-w-[150px]">
                    ...
                    <p className="text-xs font-medium text-slate-400 group-hover:text-slate-300">
                      {userRole === 'ADMIN' ? 'Administrador' : userRole === 'TESORERO' ? 'Tesorero' : userRole === 'OPERATOR' ? 'Operador' : userRole === 'SECRETARIO' ? 'Secretario' : userRole === 'VOCAL' ? 'Vocal' : 'Fiscalizador'}
                    </p>
                  </div>
                </div>
              </div>
